...
#[component]
pub fn AddLinkPage(initial_url: Option<String>) -> Element {
    let nav = navigator();
    let initial = initial_url.unwrap_or_default();

    let mut url_input = use_signal(move || initial.clone());
        ...
        spawn(async move {
            ...
            match check_duplicate_url(&url).await {
                Ok(Some(existing_link)) => {
                    // Duplicate found - navigate to edit page
                    progress_step.set(None);
                    nav.push(format!("/links/{}/edit", existing_link.id));
                }
                Ok(None) => {
    ...
    let handle_back_to_input = move |_| {
        preview.set(None);
        error.set(None);
        can_retry_preview.set(false);
    ...
    // Handle cancel - go back to links list
    ...
    rsx! {
        div {
            ...
            div { class: "content-container",
                ...
                div { class: "add-link-page-content",
